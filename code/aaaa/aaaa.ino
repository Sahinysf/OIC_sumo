...
void loop() {
  ...
  */
  if ((d > distance && e > distance && f > distance && a > distance && b > distance && c > distance)) {
    ...
    // Front Lasers
    if (d <= distance || e <= distance || f <= distance ){
     int StatusLasers2 = 0;
      if (f <= distance){StatusLasers2 += 2;}
      if (e <= distance){StatusLasers2 += 3;}
      if (d <= distance){StatusLasers2 += 7;}
      if (StatusLasers2 == 2){forward_small_right();}
      else if(StatusLasers2 == 3){forward();}
      else if(StatusLasers2 == 7){forward_small_left();}
      else{forward();} 
    }//Back Lasers
    else if(a <= distance || b <= distance || c <= distance){
      int StatusLasers = 0;
